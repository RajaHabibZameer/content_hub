...
import { X, Video, Film, Layers, Image } from 'lucide-react'
import { cn } from '@/lib/utils'

interface GenerateToolbarProps {
  selectedCount: number
  onClear: () => void
  onGenerate: (contentType: 'ugc' | 'cgi' | 'ugc_cgi' | 'image') => void
  isGenerating: boolean
}

export function GenerateToolbar({ selectedCount, onClear, onGenerate, isGenerating }: GenerateToolbarProps) {
    ...
    <div className="fixed inset-x-0 bottom-0 z-50 border-t bg-background/95 p-4 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="icon" onClick={onClear} disabled={isGenerating}>
            <X className="h-4 w-4" />
          </Button>
